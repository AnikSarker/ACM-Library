...
void dfs(int v){
    us[v] = true;
    while (g[v].size() > 0){
        int ind = g[v].back();g[v].pop_back();
        if (vis[ind])continue;
        vis[ind] = true;col[ind] = (pp ^= 1);
...
vector <int> solve(vector <edge> e){
    ...
    vector <int> l, r;
    for (auto c : e){
        l.push_back(c.a);r.push_back(c.b);
    }
    ...
    for (auto &c : e){
        ...
        g[ret.a].push_back(ind);g[ret.b].push_back(ind);ind++;
    }
    vector <int> ids;
    int mx = 0;
    for(int i = 0; i < x + y; i++){
        us[i] = 0;mx = max(mx, (int) g[i].size());
        if (g[i].size() % 2)ids.push_back(i);
    }
    bool bad = false;
    for(int i = 0; i < x + y; i++){
        if (g[i].size() > 1)bad = true;
    }
    if(!bad){vector <int> res(ind);return res;}
    else{
        vector <int> deg(x + y);
        for (int v : ids){
            if (v < x){
        ...
        if (g[x - 1].size() % 2){
            ...
            g[x - 1].push_back(ind);g[x + y - 1].push_back(ind);
            ind++;
        }
        for (int i = 0; i < ind; i++)col[i] = -1;
        for (int i = 0; i < ind; i++)vis[i] = 0;
        for (int i = 0; i < x + y; i++)if (!g[i].empty())dfs(i);
        vector <edge> to_l, to_r;
        vector <int> cols;
        for (int i = 0; i < (int) e.size(); i++){
            cols.push_back(col[i]);
            if (col[i] == 0)to_l.push_back(e[i]);
            else to_r.push_back(e[i]);
        }
        auto x = solve(to_l),y = solve(to_r);
        int mx = *max_element(x.begin(), x.end()) + 1;
        int p_x = 0, p_y = 0;
        vector <int> ans;
        for (int i = 0; i < (int) e.size(); i++){
            if (cols[i] == 0)ans.push_back(x[p_x++]);
            else ans.push_back(mx + y[p_y++]);
        }
        return ans;
    }
}
